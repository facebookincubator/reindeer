...
pub(crate) fn remove_existing_path(path: &Path) -> anyhow::Result<()> {
...
pub(crate) fn prepare_regular_file_target(path: &Path) -> anyhow::Result<()> {
    let Some(file_type) = path_file_type_no_follow(path)? else {
        return Ok(());
    };
...
pub(crate) fn write_regular_file(path: &Path, contents: &[u8]) -> anyhow::Result<()> {
...
pub(crate) fn materialization_excluded(relative: &Path, filters: &VendorFilters) -> bool {
